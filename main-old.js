...
const main = async () => {
  ...
  // mainCanvas.id = 'main'
  // const onresize = window.onresize = debounce(() => {
  //   console.log('fire onresize')
  //   mainCanvas.width = window.innerWidth * pixelRatio
  //   mainCanvas.height = window.innerHeight * pixelRatio
  // }, 500)

  // const drawCanvas = ({ canvas, rect }) => {
  //   mainCtx.drawImage(
  //     canvas,
  //     rect.x * pixelRatio,
  //     rect.y * pixelRatio
  //   )
  // }
  // const getRect = ({ width, height }, { x, y }) => {
  //   return {
  //     x, y,
  //     width,
  //     height,
  ...
  // initalize main editor
  const mainValue = await (await fetch('./dsp/test/fixtures/miss-judged.js')).text()
  const mainEditorWidth = window.innerWidth / 2.4
  const sideWidth = (window.innerWidth - mainEditorWidth) / 2

  const padding = 10
  const titlebarHeight = 42 //null//100 //null
  const fontSizeMain = '9.4pt' // '7.59pt'
  const fontSizeSide = '7.8pt' //'6.8pt'
  ...
  const mainEditor = new Editor({
    ...
    padding,
    titlebarHeight,
    width: mainEditorWidth,
    height: window.innerHeight,
  })

  const sideValue = await (await fetch('./dsp/test/fixtures/kick.js')).text()
  const sideEditor = new Editor({
    id: 'aux',
    title: 'aux',
    value: sideValue,
  ...
  const control = { el: document.createElement('div') }
  ...
  mainEditor.canvas.style.right = 0
  mainEditor.parent = container
  sideEditor.parent = container
  ...
  const onresize = window.onresize = () => {
    ...
    sideEditor.canvas.style.left = sideWidth + 'px'
    control.el.style.width = sideWidth + 'px'
  ...
  //   const faders = Array.from(Array(8), () => ({
  //     vol: .75
  //   }))
  ...
  //     let h = height - 22
  //     ctx.fillStyle = '#222'
  //     ctx.fillRect(x, 5, 30, h)
  //     ctx.fillStyle = '#444'
  //     let top = (h * (1-faders[i].vol))
  //     ctx.fillRect(x, 5 + top, 30, h - top)
  //     // ctx.fillStyle = '#f00'
  //     ctx.clearRect(x, h + 5, 30, 20)
  //     ctx.fillStyle = faders[i].solo ? '#fff' : '#666'
  //     ctx.fillText('S', x+3, 7 + h)
  //     ctx.fillStyle = '#fff'
  //     ctx.fillText(i, x + 12, 7 + h)
  ...
  //     // x -= 2.5
  //     // console.log(x, y)
  //     let i = Math.ceil((x - 5) / pixelRatio / 35) - 1
  //     let vol = Math.max(0, Math.min(1, 1 - ((y-15) / (78 * pixelRatio))))
  //     if (faders[i]) {
  //       faders[i].vol = vol
  //       drawFader(i)
  //     }
